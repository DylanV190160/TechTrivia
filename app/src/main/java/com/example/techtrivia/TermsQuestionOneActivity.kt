package com.example.techtrivia

import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.view.View
import android.widget.RadioButton
import android.widget.Toast
import kotlinx.android.synthetic.main.activity_terms_question_one.*

class TermsQuestionOneActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {

        window.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_FULLSCREEN

        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_terms_question_one)

        val terminologyQuestionsList = Constants.getTerminologyQuestions()
        val questionNumber: Int = 1
        val question: Question = terminologyQuestionsList[0]
        //Set UI elements
        tv_question_terms.text = question.question
        rb_answer_one_terms.text = question.optionOne
        rb_answer_two_terms.text = question.optionTwo
        rb_answer_three_terms.text = question.optionThree
        rb_answer_four_terms.text = question.optionFour
        pb_progressBar_terms.progress = questionNumber
        tv_progress_terms.text = "1" + "/" + terminologyQuestionsList.size

        var answers: RadioButton
        var termsCorrectAnswers: Int = 0

        btn_next_terms.setOnClickListener{
            var id: Int = rg_options_terms.checkedRadioButtonId

            if (id!=-1) {
                answers = findViewById(id)

                Toast.makeText(this, "Checked answer: ${answers.text}", Toast.LENGTH_SHORT).show()
                if (answers.text === question.optionThree) {
                    termsCorrectAnswers++
                }
                intent = Intent(this, TermsQuestionTwoActivity::class.java)
                intent.putExtra(Constants.CORRECT_ANSWERS_TERMS, termsCorrectAnswers)
                startActivity(intent)
                finish()

            } else {
                Toast.makeText(this, "Please select your answer!", Toast.LENGTH_SHORT).show()
            }
        }
    }
}